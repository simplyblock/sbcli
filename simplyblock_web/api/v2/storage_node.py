...
def add(cluster: Cluster, parameters: StorageNodeParams) -> Response:
    ...
    task_url = task_instance_api.url_path_for('get', cluster_id=cluster.get_id(), task_id=task_id_or_false)
...
def _storage_node_lookup(storage_node_id: Annotated[str, StringConstraints(pattern=core_utils.UUID_PATTERN)]) -> StorageNodeModel:
    storage_node = db.get_storage_node_by_id(storage_node_id)
