...
def list(
        cluster: Cluster,
        severity: Annotated[AlertSeverity | None, Query(
            description="Only return alerts of this severity")] = None,
        history: bool = Query(
            False,
            description="Also return alerts that have already resolved"),
        history_seconds: int | None = Query(
            None, ge=1,
            description="Limit the history to alerts resolved within this many "
                        "seconds. Implies history=true."),
        status: Annotated[AlertStatus | None, Query(
            description="Only return alerts in this state")] = None,
) -> builtins.list[AlertDTO]:
