...
@api.get('/', name='clusters:tasks:list', response_model=builtins.list[TaskDTO], responses=WATCH_RESPONSES)
def list(cluster: Cluster, watch: WatchParam = False) -> Union[builtins.list[TaskDTO], EventSourceResponse]:
    if watch:
        return sse_response(
