...
def add(request: Request, cluster: Cluster, parameters: PoolParams) -> Response:
    ...
    entity_url = request.app.url_path_for('clusters:pools:detail', cluster_id=cluster.get_id(), pool_id=id_or_false)
...
def _lookup_pool(pool_id: UUID) -> PoolModel:
...
class UpdatablePoolParams(BaseModel):
    ...
    lvol_max_size: Optional[util.Unsigned] = None
