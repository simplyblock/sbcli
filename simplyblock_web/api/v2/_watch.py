...
def _build_diff(
        prev: RawState, new: RawState, new_full: RawState, cache: Dict[Any, str],
    ...
    for key in prev:
        if key in new:
            continue
        # The entity left the projected set. If its key is still retrievable
        # in the unprojected state (e.g. an LVol whose status flipped to
        # 'deleted', which the list filter excludes), the event carries the
        # DTO of that *current* state — not the stale last-projected one.
        # A key absent from the unprojected state is physically gone, so
        # there is no representation left to return. The lookup runs in the
        # raw keyspace: projections that transform keys (exploded devices)
        # resolve to "gone", which is correct — a device that left its
        # node's list has no DB representation either.
        current = new_full.get(key)
        if current is not None:
            try:
                data = build_dto(current).model_dump_json()
            except Exception:
                logger.exception('Failed to build DTO for deleted entity %s', key)
                data = cache.get(key, '{}')
        else:
            data = '{}'
        events.append(ServerSentEvent(event='deleted', data=data))
...
def watch_response(
    ...
    emit ``created``/``deleted``. ``deleted`` carries the entity's current
    (unprojected) state while its key is still retrievable, else ``{}`` —
    see ``_build_diff``. ``single_id`` switches to detail semantics
        ...
        try:
            ...
            while True:
                ...
                if prev is None:
                    ...
                    events, dto_cache = await run_in_threadpool(
                        _build_diff, prev, filtered, state, dto_cache, build_dto, single)
                prev = filtered
                for event in events:
                    yield event
                if single and not filtered:
