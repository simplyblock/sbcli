...
class SoakRunner:
    ...
    def _start_fio_for_volume(self, volume, fio_name):
        ...
        start_script = (
            ...
            + shlex.quote(
                ...
                "--verify=md5 --do_verify=1 --verify_fatal=1 "
    ...
    def _validate_topology_for_categories(self):
        ...
        The "unrelated" category is soft: in a dense FT=2 ring with N ≤ 4
        every pair shares at least one LVS, so no unrelated pair exists.
        Log a warning so the gap is visible; pick_pair_for_category will
        fall back to the next-widest distance at iteration time.
        """
        ...
        if not self._unrelated_pairs(all_nodes, lvs_members):
            self.logger.log(
                "WARN: No unrelated node pair available in topology "
                f"({len(all_nodes)} nodes across {len(lvs_members)} LVSs); "
                "'unrelated' scenarios will fall back to primary_tertiary, "
    ...
    def pick_pair_for_category(self, category):
        ...
        "unrelated" is best-effort: dense FT=2 rings with N ≤ 4 cannot
        supply any unrelated pair. In that case fall back to the next-
        widest distance (primary_tertiary, then primary_secondary) so the
        scenario still runs.
        """
        if category == "unrelated":
            ...
            if not candidates:
                for fallback in ("tertiary", "secondary"):
                    candidates = self._candidate_pairs_for_role(fallback)
                    if candidates:
                        self.logger.log(
                            f"unrelated: no unrelated pair available; "
                            f"falling back to primary_{fallback} pair"
                        )
                        break
