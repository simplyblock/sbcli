...
run_fio_workload() {
    ...
    sudo fio --directory=$mount_point --readwrite=write --bs=64K --size=$size --name=test --numjobs=1 \
        --nrfiles=$nrfiles --direct=1 --ioengine=aiolib --iodepth=1 --verify=md5 \
...
ALL_LVOLS=($($SBCLI_CMD lvol list | grep -i lvol | awk '{print $4}'))
for lvol_name in "${ALL_LVOLS[@]}"; do
    mount_point="$MOUNT_DIR/$lvol_name/extra"
