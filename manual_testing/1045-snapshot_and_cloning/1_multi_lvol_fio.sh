...
# Variables
SBCLI_CMD="sbcli-new"
CN_ID="67663ea2-42c1-4dd6-9fba-38830582cae7"
POOL_NAME="pool1"
LVOL_SIZE="160G"
...
create_lvol() {
    ...
    cmd="$SBCLI_CMD lvol add --distr-ndcs $ndcs --distr-npcs $npcs --max-size $LVOL_SIZE --snapshot $name $LVOL_SIZE $pool"
...
connect_lvol() {
    ...
    connect_command="$SBCLI_CMD -d cn connect $CN_ID $lvol_id"
...
disconnect_lvol() {
    ...
    cmd="$SBCLI_CMD -d cn disconnect $CN_ID $lvol_id"
    echo $cmd
    $cmd
}
...
for fs_type in "${FS_TYPES[@]}"; do
    ...
    for size in "${WORKLOAD_SIZE[@]}"; do
        ...
        for i in "${!CONFIGS[@]}"; do
            ...
            lvol_id=$($SBCLI_CMD lvol list | grep -i $lvol_name | awk '{print $2}')
            $SBCLI_CMD snapshot add $lvol_id $snapshot_name
            snapshot_names_arr[$i]=$snapshot_name
        done

        log "Listing snapshots"
        $SBCLI_CMD snapshot list
        pause_if_interactive_mode "$@"
        ...
        for i in "${!CONFIGS[@]}"; do
            ...
            snapshot_id=$($SBCLI_CMD snapshot list | grep -i $snapshot_name | awk '{print $2}')
            clone_name="${snapshot_name}_cl"
            $SBCLI_CMD snapshot clone $snapshot_id $clone_name
        ...
        for i in "${!CONFIGS[@]}"; do
            ...
            clone_id=$($SBCLI_CMD lvol list | grep -i $clone_name | awk '{print $2}')
            ...
            if [ "$fs_type" == "xfs" ]; then
                cmd="sudo xfs_admin -U generate /dev/$clone_device"
                log "Run command (before mounting dev formatted in xfs: $cmd"
                $cmd
                log "Mounting clone device: /dev/$clone_device at $clone_mount_point"
                echo "sudo mount /dev/$clone_device $clone_mount_point"
                sudo mount /dev/$clone_device $clone_mount_point -o nouuid
                pause_if_interactive_mode "$@"
            else
                log "Mounting clone device: /dev/$clone_device at $clone_mount_point"
                echo "sudo mount /dev/$clone_device $clone_mount_point"
        ...
        for i in "${!CONFIGS[@]}"; do
            ...
            $SBCLI_CMD lvol delete $clone_id
