...
def health_check(node_id):
    ...
    try:
        logger.info("Connecting to node's docker")
        node_docker = docker.DockerClient(base_url=f"tcp://{snode.mgmt_ip}:2375", version="auto")
        ...
        for cont in containers_list:
            ...
            if name in ['/spdk', '/spdk_proxy', '/SNodeAPI'] \
                    or name.startswith("/app_PortStatsCollector") \
                    or name.startswith("/app_DistrEventCollector"):
                logger.debug(state)
                since = ""
                try:
                    ...
                    pass
                logger.info(f"Container: {name}, Status: {state['Status']}, Since: {since}")
