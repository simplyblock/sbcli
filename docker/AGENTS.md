...
- **`env_var` must stay beside the package.** `constants.py`'s `get_config_var` opens
  `f"{SCRIPT_PATH}/env_var"`, and `release.yml` / `python-publish.yml` `sed` that file.
...
- **The `python3-*` removals are required.** `requirements.txt` installs `urllib3`,
  `requests`, `idna` and `charset-normalizer` from PyPI; pip cannot uninstall an RPM-provided
  distribution (no `RECORD` file) and fails the build, and merely shadowing them would leave
  stale versions visible to the RPM-database scanners in `security.yml`. If a future UBI adds
  another such duplicate, the build fails loudly with `Cannot uninstall <pkg>` — add it to the
  removal list rather than reaching for `--ignore-installed`.
