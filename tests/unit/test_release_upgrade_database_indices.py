...
class _Phase:
    ...
    def __enter__(self):
        ...
        self._patches = [
            ...
            patch.object(database_indices.index_ops, 'backfill_lvol_cluster_id',
                         return_value='filled'),
        ]
        for p in self._patches:
            p.start()
        self.build_indices = database_indices.index_ops.build_indices
        self.backfill = database_indices.index_ops.backfill_lvol_cluster_id
...
class TestPreUpdate(unittest.TestCase):
    ...
    def test_builds_nothing(self):
        ...
        with _Phase() as phase:
            ...
            phase.backfill.assert_not_called()
...
class TestUpgradeComplete(unittest.TestCase):
    ...
    def test_backfills_and_flips(self):
        with _Phase() as phase:
            messages = phase.plugin.upgrade_complete(phase.cluster)
            phase.backfill.assert_called_once()
