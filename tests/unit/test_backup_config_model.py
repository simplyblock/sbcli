...
class TestBackupLocation:
    ...
    @pytest.mark.parametrize("missing", ["bucket_name", "region"])
    def test_mandatory_fields(self, missing):
        data = {k: v for k, v in MINIMAL.items() if k != missing}
        with pytest.raises(ValidationError):
            BackupLocation.model_validate(data)

    @pytest.mark.parametrize("field", ["bucket_name", "region"])
    def test_empty_string_is_not_a_value(self, field):
        """The models must not accept "" as a stand-in for "unset"."""
        with pytest.raises(ValidationError):
            BackupLocation.model_validate({**MINIMAL, field: ""})
...
class TestSecondaryTarget:
    @pytest.mark.parametrize(
        "target,wire", [(SecondaryTarget.S3, 0), (SecondaryTarget.FILESYSTEM, 1)]
    )
    def test_wire_round_trip(self, target, wire):
        assert target.wire_value == wire
        assert SecondaryTarget.from_wire(wire) is target

    def test_unknown_wire_value(self):
        with pytest.raises(ValueError):
            SecondaryTarget.from_wire(7)
...
class TestLegacyMigration:
    ...
    def test_legacy_config_without_a_region_is_rejected(self):
        """A backup whose region was never recorded is not restorable elsewhere."""
        with pytest.raises(ValidationError):
...
class TestClusterAccessor:
    ...
    def test_unconfigured_cluster_raises(self):
        ...
        with pytest.raises(PreconditionError, match="no backup configuration"):
            cluster.get_backup_config()

    def test_invalid_config_raises_precondition_not_validation_error(self):
        """Callers guard on PreconditionError; a pydantic error would escape them."""
        cluster = Cluster()
        cluster.backup_config = {"bucket_name": "backups"}  # no region
        with pytest.raises(PreconditionError, match="invalid backup configuration"):
            cluster.get_backup_config()
