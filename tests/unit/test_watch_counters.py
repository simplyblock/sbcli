...
def test_scope_derivation_per_class():
    ...
    assert EventObj({'uuid': 'e', 'cluster_uuid': 'c', 'date': 1}).watch_scope() == ('c',)
...
def test_event_write_indexes_by_get_id():
    """EventObj's compound get_id() (cluster/date/uuid) is the version leaf,
    same shape as JobSchedule -- this is what lets a tailed watch (an
    otherwise-unbounded per-cluster log) bound its scan by reverse-sorting on
    that leaf instead of holding the whole history."""
    kv = MagicMock()
    event = EventObj({'uuid': 'ev-1', 'cluster_uuid': 'cl-1', 'date': 42})
    event.write_to_db(kv)
    key, _value = kv.set.call_args[0]
    assert key == b'object/EventObj/cl-1/42/ev-1'
    kv.add.assert_any_call(b'watch_index/EventObj/cl-1', watches.ONE_LE64)
    kv.add.assert_any_call(b'watch_index/EventObj/cl-1/cl-1/42/ev-1', watches.ONE_LE64)
