...
def test_events_created_updated_deleted():
    ...
    assert '"y"' in by_name['updated'].data and by_name['updated'].id == 'a'
    assert '"c"' in by_name['created'].data and by_name['created'].id == 'c'
    assert by_name['deleted'].data == '{}' and by_name['deleted'].id == 'b'  # physically gone: no representation
    assert deleted is True
    assert set(new_cache) == {'a', 'c'}


def test_events_deleted_with_no_model_still_carries_id():
    """A list-mode delete with no representation left (physically gone, or --
    for a tailed scope -- aged out of the window) must still identify which
    entity to drop: the SSE wire `id:` field is the only place left to put it
    once `data` is the empty-object placeholder."""
    events, _, _ = _build_events([_deleted('b')], {}, _dto, single=False)
    [event] = events
    assert event.data == '{}'
    assert event.id == 'b'


def test_snapshot_events_carry_id():
    events, _ = _build_snapshot([_created('a')], _dto, single=True)
    [event] = events
    assert event.id == 'a'
