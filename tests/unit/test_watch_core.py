...
def test_scopewatch_tail_bounds_to_recent_n():
    """A tail-bounded ScopeWatch (an unbounded scope like a per-cluster event
    log) only ever holds its N most-recently-changed leaves: the reverse+limit
    scan itself keeps aged-out leaves out of `cur`, so they fall through the
    existing `removed` path exactly like a physical delete would."""
    kv = FakeKV()
    sw = ScopeWatch(Pool, ('cl-1',), None, tail=2)

    for i, leaf in enumerate(['p1', 'p2', 'p3'], start=1):
        pool = Pool({'uuid': leaf, 'cluster_id': 'cl-1', 'pool_name': leaf})
        _write_obj(kv, pool)
        _set_version(kv, Pool, ('cl-1',), leaf, i)

    entities, changed = sw._read(kv)
    assert changed and sorted(p.get_id() for p in entities) == ['p2', 'p3']

    pool4 = Pool({'uuid': 'p4', 'cluster_id': 'cl-1', 'pool_name': 'p4'})
    _write_obj(kv, pool4)
    _set_version(kv, Pool, ('cl-1',), 'p4', 4)
    entities, changed = sw._read(kv)
    assert changed and sorted(p.get_id() for p in entities) == ['p3', 'p4']  # 'p2' aged out
...
def _patch(monkeypatch, watches_map):
    monkeypatch.setattr(
        ...
        lambda cls, scope=(), entity_id=None, tail=None: watches_map[(cls, tuple(scope), entity_id)])
