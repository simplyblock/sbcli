...
pool — including the one being resized — plus snapshot used bytes. The old
check ``total + new_size > pool_max_size`` therefore counted that volume
twice (old size + new size): a single 4G volume in a 10G pool could not be
resized past 6G. The correct admission is
...
def _resize(lvol_size, pool_total, pool_max, new_size):
    ...
    with patch.object(lvol_controller, "DBController") as db_cls, \
        ...
        return lvol_controller.resize_lvol("lv-1", new_size)
...
class TestResizeDoesNotDoubleCountItself(unittest.TestCase):
    ...
    def test_growth_past_cap_is_rejected(self):
        ...
        this branch returns (False, msg) instead of raising."""
        ret, msg = _resize(lvol_size=4 * G, pool_total=8 * G,
                           pool_max=10 * G, new_size=6 * G + 1)
        self.assertFalse(ret)
        self.assertIn("Pool max size", msg)
