...
from simplyblock_core.cluster_ops import EVENT_LOG_TAIL
...
class SSEReader:
    ...
    def __init__(self, response):
        ...
        self._buffer = ''
        self.last_id = None  # SSE `id:` field of the most recently returned event
    ...
    def next_event(self):
        ...
        Also stashes the SSE `id:` field (if any) on ``self.last_id``.
        """
...
def _seed_event(db, cluster, **overrides):
    event = EventObj({
        'uuid': str(uuid4()),
        'cluster_uuid': cluster.get_id(),
...
def _logs_watch_url(base_url, cluster):
    return f'{base_url}/api/v2/clusters/{cluster.get_id()}/logs?watch=true'
...
@pytest.mark.timeout(120)
def test_cluster_logs_watch_tail_eviction(base_url):
    """Cluster logs are an unbounded, append-only scope: the watch must stay
    bounded to EVENT_LOG_TAIL entries, and an entry that ages out of that
    window must arrive as a 'deleted' frame identifying it via the SSE `id:`
    field (there is no representation left to put in `data`)."""
    db = DBController()
    cluster = _seed_cluster(db)
    events = [
        _seed_event(db, cluster, date=1_700_000_000_000 + i, message=f'event-{i}')
        for i in range(EVENT_LOG_TAIL)
    ]

    with requests.get(_logs_watch_url(base_url, cluster),
                      stream=True, timeout=(5, 30)) as response:
        assert response.status_code == 200
        reader = SSEReader(response)
        ...
        assert name == 'snapshot'
        snapshot = json.loads(data)
        assert len(snapshot) == EVENT_LOG_TAIL
        assert {e['id'] for e in snapshot} == {ev.uuid for ev in events}

        oldest, newest = events[0], _seed_event(
            db, cluster, date=1_700_000_000_000 + EVENT_LOG_TAIL, message='newest')

        name, data = reader.next_event()
        assert name == 'created'
        assert json.loads(data)['id'] == newest.uuid

        name, data = reader.next_event()
        assert name == 'deleted'
        assert data == '{}'  # aged out, not physically removed: no representation
        # The SSE `id:` is the ChangeEvent key (get_id()), stable across an
        # entity's whole created/updated/deleted lifecycle -- for EventObj's
        # compound key that's "cluster/date/uuid", not the bare uuid in the
        # DTO body.
        assert reader.last_id == oldest.get_id()
