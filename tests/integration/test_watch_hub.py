# coding=utf-8
"""EntityWatchHub against real FDB: watch firing, reconcile poll, teardown."""
...
from simplyblock_web.api.v2._watch import _UNSET, EntityWatchHub
...
def test_hub_publishes_writes_and_stops_on_unsubscribe():
    ...
    async def scenario():
        ...
        try:
            ...
            assert any(d.get('uuid') == pool.get_id() for d in state.values())

            await asyncio.to_thread(pool.remove, db.kv_store)
            state = await _await_publication(sub, notify, timeout=5.0)
            assert not any(d.get('uuid') == pool.get_id() for d in state.values())
        finally:
            hub.unsubscribe(sub)
...
def test_reconcile_catches_writes_without_counter_bump(monkeypatch):
    ...
    monkeypatch.setattr(_watch, 'WATCH_RECONCILE_SEC', 0.5)
    db = DBController()
    ...
    async def scenario():
        ...
        try:
            ...
            assert any(d.get('uuid') == legacy['uuid'] for d in state.values())
        finally:
            hub.unsubscribe(sub)
