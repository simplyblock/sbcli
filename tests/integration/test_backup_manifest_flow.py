...
class TestBuildManifest:
    ...
    def test_records_the_full_chain_oldest_first(self, db, cluster, lvol):
        _backup(db, "b-1", 1)
        ...
        assert [(e.backup_id, e.s3_id) for e in manifest.chain] == [
            ("b-1", 1), ("b-2", 2), ("b-3", 3)]

    def test_chain_lets_a_restore_avoid_walking_manifests(self, db, cluster, lvol):
        """The whole chain is in one object, so one read is enough to plan a restore."""
        _backup(db, "b-1", 1)
        manifest = backup_controller.build_manifest(_backup(db, "b-2", 2, prev="b-1"))

        assert manifest.chain[-1].backup_id == manifest.backup_id
    ...
    def test_survives_a_deleted_volume(self, db, cluster, lvol):
        ...
        assert manifest.volume.pool_name == ""      # only on the volume
...
class TestExportImportRoundTrip:
    ...
    def test_export_emits_the_same_shape_as_the_bucket(self, db, cluster, lvol):
        ...
        parsed = backup_manifest.BackupManifest.model_validate(exported[0])
        assert parsed.backup_id == "b-1"
    ...
    def test_malformed_manifest_rejects_the_whole_batch(self, db, cluster, lvol):
        _backup(db, "b-1", 1)
        exported = backup_controller.export_backups(cluster_id=CLUSTER_ID)
        for backup in db.get_backups():
            backup.remove(db.kv_store)

        with pytest.raises(PreconditionError, match="malformed"):
            backup_controller.import_backups(
                exported + [{"backup_id": "b-9", "s3_id": "not-an-int"}],
    ...
    def test_same_id_listed_twice_rejects_the_whole_batch(self, db, cluster, lvol):
        ...
        with pytest.raises(PreconditionError, match="listed more than once"):
            backup_controller.import_backups(exported + exported, cluster_id="cluster-2")

        assert db.get_backups() == []

    def test_entry_without_a_backup_id_is_skipped(self, db, cluster, lvol):
        assert backup_controller.import_backups([{"s3_id": 1}], cluster_id="cluster-2") == 0
