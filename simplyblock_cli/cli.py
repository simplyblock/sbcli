...
class CLIWrapper(CLIWrapperBase):
    ...
    def init_storage_node__add_node(self, subparser):
        ...
        argument = subcommand.add_argument('--jm-percent', help='Number in percent to use for JM from each device' if self.developer_mode else argparse.SUPPRESS, type=int, default=3, dest='jm_percent', required=False)
        ...
        argument = subcommand.add_argument('--number-of-distribs', help='The number of distirbs to be created on the node' if self.developer_mode else argparse.SUPPRESS, type=int, default=4, dest='number_of_distribs', required=False)
        ...
        argument = subcommand.add_argument('--size-of-device', help='Size of device per storage node' if self.developer_mode else argparse.SUPPRESS, type=str, dest='partition_size', required=False)
        ...
        argument = subcommand.add_argument('--cpu-mask', help='SPDK app CPU mask, default is all cores found' if self.developer_mode else argparse.SUPPRESS, type=str, dest='spdk_cpu_mask', required=False)
        argument = subcommand.add_argument('--spdk-image', help='SPDK image uri' if self.developer_mode else argparse.SUPPRESS, type=str, dest='spdk_image', required=False)
        argument = subcommand.add_argument('--spdk-debug', help='Enable spdk debug logs' if self.developer_mode else argparse.SUPPRESS, dest='spdk_debug', required=False, action='store_true')
        argument = subcommand.add_argument('--iobuf_small_bufsize', help='bdev_set_options param' if self.developer_mode else argparse.SUPPRESS, type=int, default=0, dest='small_bufsize', required=False)
        argument = subcommand.add_argument('--iobuf_large_bufsize', help='bdev_set_options param' if self.developer_mode else argparse.SUPPRESS, type=int, default=0, dest='large_bufsize', required=False)
        argument = subcommand.add_argument('--enable-test-device', help='Enable creation of test device' if self.developer_mode else argparse.SUPPRESS, dest='enable_test_device', required=False, action='store_true')
        argument = subcommand.add_argument('--disable-ha-jm', help='Disable HA JM for distrib creation' if self.developer_mode else argparse.SUPPRESS, dest='enable_ha_jm', required=False, action='store_true')
        ...
        argument = subcommand.add_argument('--id-device-by-nqn', help='Use device nqn to identify it instead of serial number' if self.developer_mode else argparse.SUPPRESS, dest='id_device_by_nqn', required=False, action='store_true')
        argument = subcommand.add_argument('--max-snap', help='Max snapshot per storage node' if self.developer_mode else argparse.SUPPRESS, type=int, default=5000, dest='max_snap', required=False)
    ...
    def init_storage_node__restart(self, subparser):
        ...
        argument = subcommand.add_argument('--max-snap', help='Max snapshot per storage node' if self.developer_mode else argparse.SUPPRESS, type=int, default=5000, dest='max_snap', required=False)
        argument = subcommand.add_argument('--max-size', help='Maximum amount of GB to be utilized on this storage node' if self.developer_mode else argparse.SUPPRESS, type=str, default='', dest='max_prov', required=False)
    ...
    def init_cluster__deploy(self, subparser):
        ...
        argument = subcommand.add_argument('--ha-jm-count', help='HA JM count' if self.developer_mode else argparse.SUPPRESS, type=int, default=3, dest='ha_jm_count', required=False)
        ...
        argument = subcommand.add_argument('--enable-qos', help='Enable qos bdev for storage nodes' if self.developer_mode else argparse.SUPPRESS, dest='enable_qos', required=False, action='store_true')
        argument = subcommand.add_argument('--ifname', help='Management interface name, e.g. eth0', type=str, dest='ifname', required=False)
        argument = subcommand.add_argument('--blk_size', help='The block size in bytes' if self.developer_mode else argparse.SUPPRESS, type=int, default=512, dest='blk_size', required=False, choices=['512','4096',])
        argument = subcommand.add_argument('--page_size', help='The size of a data page in bytes' if self.developer_mode else argparse.SUPPRESS, type=int, default=2097152, dest='page_size', required=False)
        argument = subcommand.add_argument('--CLI_PASS', help='Password for CLI SSH connection' if self.developer_mode else argparse.SUPPRESS, type=str, dest='CLI_PASS', required=False)
        ...
        argument = subcommand.add_argument('--grafana-endpoint', help='Endpoint url for Grafana' if self.developer_mode else argparse.SUPPRESS, type=str, default='', dest='grafana_endpoint', required=False)
        argument = subcommand.add_argument('--distr-bs', help='(Dev) distrb bdev block size, default: 4096' if self.developer_mode else argparse.SUPPRESS, type=int, default=4096, dest='distr_bs', required=False)
        ...
        argument = subcommand.add_argument('--max-queue-size', help='The max size the queue will grow' if self.developer_mode else argparse.SUPPRESS, type=int, default=128, dest='max_queue_size', required=False)
        argument = subcommand.add_argument('--inflight-io-threshold', help='The number of inflight IOs allowed before the IO queuing starts' if self.developer_mode else argparse.SUPPRESS, type=int, default=4, dest='inflight_io_threshold', required=False)
        ...
        argument = subcommand.add_argument('--iobuf_large_bufsize', help='bdev_set_options param' if self.developer_mode else argparse.SUPPRESS, type=int, default=0, dest='large_bufsize', required=False)
        argument = subcommand.add_argument('--enable-test-device', help='Enable creation of test device' if self.developer_mode else argparse.SUPPRESS, dest='enable_test_device', required=False, action='store_true')
        argument = subcommand.add_argument('--disable-ha-jm', help='Disable HA JM for distrib creation' if self.developer_mode else argparse.SUPPRESS, dest='enable_ha_jm', required=False, action='store_false')
        argument = subcommand.add_argument('--is-secondary-node', help='Adds as secondary node. A secondary node does not have any disks attached. It is only used for I/O processing in case a primary goes down.', dest='is_secondary_node', required=False, action='store_true')
        ...
        argument = subcommand.add_argument('--lvol-name', help='Logical volume name or id' if self.developer_mode else argparse.SUPPRESS, type=str, default='lvol01', dest='lvol_name', required=False)
        argument = subcommand.add_argument('--lvol-size', help='Logical volume size: 10M, 10G, 10(bytes)' if self.developer_mode else argparse.SUPPRESS, type=str, default='10G', dest='lvol_size', required=False)
        argument = subcommand.add_argument('--pool-name', help='Pool id or name' if self.developer_mode else argparse.SUPPRESS, type=str, default='pool01', dest='pool_name', required=False)
        argument = subcommand.add_argument('--pool-max', help='Pool maximum size: 20M, 20G, 0(default)' if self.developer_mode else argparse.SUPPRESS, type=str, default='25G', dest='pool_max', required=False)
        argument = subcommand.add_argument('--snapshot', help='Make logical volume with snapshot capability, default: false' if self.developer_mode else argparse.SUPPRESS, dest='snapshot', required=False, action='store_true')
        argument = subcommand.add_argument('--max-volume-size', help='Logical volume max size' if self.developer_mode else argparse.SUPPRESS, type=str, default='1000G', dest='max_size', required=False)
        argument = subcommand.add_argument('--host-id', help='Primary storage node id or hostname', type=str, dest='host_id', required=False)
        argument = subcommand.add_argument('--encrypt', help='Use inline data encryption and decryption on the logical volume' if self.developer_mode else argparse.SUPPRESS, dest='encrypt', required=False, action='store_true')
        argument = subcommand.add_argument('--crypto-key1', help='Hex value of key1 to be used for logical volume encryption' if self.developer_mode else argparse.SUPPRESS, type=str, dest='crypto_key1', required=False)
        argument = subcommand.add_argument('--crypto-key2', help='Hex value of key2 to be used for logical volume encryption' if self.developer_mode else argparse.SUPPRESS, type=str, dest='crypto_key2', required=False)
        argument = subcommand.add_argument('--max-rw-iops', help='Maximum Read Write IO Per Second' if self.developer_mode else argparse.SUPPRESS, type=int, dest='max_rw_iops', required=False)
        argument = subcommand.add_argument('--max-rw-mbytes', help='Maximum Read Write Megabytes Per Second' if self.developer_mode else argparse.SUPPRESS, type=int, dest='max_rw_mbytes', required=False)
        argument = subcommand.add_argument('--max-r-mbytes', help='Maximum Read Megabytes Per Second' if self.developer_mode else argparse.SUPPRESS, type=int, dest='max_r_mbytes', required=False)
        argument = subcommand.add_argument('--max-w-mbytes', help='Maximum Write Megabytes Per Second' if self.developer_mode else argparse.SUPPRESS, type=int, dest='max_w_mbytes', required=False)
        argument = subcommand.add_argument('--distr-vuid', help='(Dev) set vuid manually, default: random (1-99999)' if self.developer_mode else argparse.SUPPRESS, type=int, dest='distr_vuid', required=False)
        argument = subcommand.add_argument('--lvol-ha-type', help='Logical volume HA type (single, ha), default is cluster HA type' if self.developer_mode else argparse.SUPPRESS, type=str, default='default', dest='lvol_ha_type', required=False, choices=['single','default','ha',])
        ...
        argument = subcommand.add_argument('--fstype', help='Filesystem type for testing (ext4, xfs)' if self.developer_mode else argparse.SUPPRESS, type=str, default='xfs', dest='fstype', required=False, choices=['ext4','xfs',])
    ...
    def init_cluster__create(self, subparser):
        ...
        argument = subcommand.add_argument('--parity-chunks-per-stripe', help='Erasure coding schema parameter n (distributed raid), default: 1', type=int, default=1, dest='distr_npcs', required=False)
        argument = subcommand.add_argument('--distr-bs', help='(Dev) distrb bdev block size, default: 4096' if self.developer_mode else argparse.SUPPRESS, type=int, default=4096, dest='distr_bs', required=False)
        argument = subcommand.add_argument('--distr-chunk-bs', help='(Dev) distrb bdev chunk block size, default: 4096' if self.developer_mode else argparse.SUPPRESS, type=int, default=4096, dest='distr_chunk_bs', required=False)
        argument = subcommand.add_argument('--ha-type', help='Logical volume HA type (single, ha), default is cluster ha type', type=str, default='ha', dest='ha_type', required=False, choices=['single','ha',])
        argument = subcommand.add_argument('--enable-node-affinity', help='Enable node affinity for storage nodes', dest='enable_node_affinity', required=False, action='store_true')
        ...
        argument = subcommand.add_argument('--max-queue-size', help='The max size the queue will grow' if self.developer_mode else argparse.SUPPRESS, type=int, default=128, dest='max_queue_size', required=False)
        argument = subcommand.add_argument('--inflight-io-threshold', help='The number of inflight IOs allowed before the IO queuing starts' if self.developer_mode else argparse.SUPPRESS, type=int, default=4, dest='inflight_io_threshold', required=False)
        argument = subcommand.add_argument('--enable-qos', help='Enable qos bdev for storage nodes, true by default' if self.developer_mode else argparse.SUPPRESS, type=bool, default=False, dest='enable_qos', required=False)
        argument = subcommand.add_argument('--strict-node-anti-affinity', help='Enable strict node anti affinity for storage nodes. Never more than one chunk is placed on a node. This requires a minimum of _data-chunks-in-stripe + parity-chunks-in-stripe + 1_ nodes in the cluster.', dest='strict_node_anti_affinity', required=False, action='store_true')
    ...
    def init_cluster__add(self, subparser):
        ...
        argument = subcommand.add_argument('--max-queue-size', help='The max size the queue will grow' if self.developer_mode else argparse.SUPPRESS, type=int, default=128, dest='max_queue_size', required=False)
        argument = subcommand.add_argument('--inflight-io-threshold', help='The number of inflight IOs allowed before the IO queuing starts' if self.developer_mode else argparse.SUPPRESS, type=int, default=4, dest='inflight_io_threshold', required=False)
        argument = subcommand.add_argument('--enable-qos', help='Enable qos bdev for storage nodes, default: true' if self.developer_mode else argparse.SUPPRESS, type=bool, default=False, dest='enable_qos', required=False)
        argument = subcommand.add_argument('--strict-node-anti-affinity', help='Enable strict node anti affinity for storage nodes. Never more than one chunk is placed on a node. This requires a minimum of _data-chunks-in-stripe + parity-chunks-in-stripe + 1_ nodes in the cluster."', dest='strict_node_anti_affinity', required=False, action='store_true')
    ...
    def init_volume__add(self, subparser):
        ...
        argument = subcommand.add_argument('--uid', help='Set logical volume id' if self.developer_mode else argparse.SUPPRESS, type=str, dest='uid', required=False)
    ...
    def init_storage_pool__add(self, subparser):
        ...
        argument = subcommand.add_argument('--max-r-mbytes', help='Maximum Read Megabytes Per Second', type=int, dest='max_r_mbytes', required=False)
        argument = subcommand.add_argument('--max-w-mbytes', help='Maximum Write Megabytes Per Second', type=int, dest='max_w_mbytes', required=False)
        argument = subcommand.add_argument('--has-secret', help='Pool is created with a secret (all further API interactions with the pool and logical volumes in the pool require this secret)' if self.developer_mode else argparse.SUPPRESS, dest='has_secret', required=False, action='store_true')
    ...
    def init_caching_node__add_node(self, subparser):
        ...
        argument = subcommand.add_argument('--vcpu-count', help='Number of vCPUs used for SPDK. Remaining CPUs will be used for Linux system, TCP/IP processing, and other workloads. The default on non-Kubernetes hosts is 80%%.', type=int, dest='vcpu_count', required=False)
        argument = subcommand.add_argument('--cpu-mask', help='SPDK app CPU mask, default is all cores found' if self.developer_mode else argparse.SUPPRESS, type=str, dest='spdk_cpu_mask', required=False)
        argument = subcommand.add_argument('--memory', help='SPDK huge memory allocation. By default it will acquire all available huge pages.' if self.developer_mode else argparse.SUPPRESS, type=int, dest='spdk_mem', required=False)
        argument = subcommand.add_argument('--spdk-image', help='SPDK image uri' if self.developer_mode else argparse.SUPPRESS, type=str, dest='spdk_image', required=False)
        argument = subcommand.add_argument('--namespace', help='k8s namespace to deploy on', type=str, dest='namespace', required=False)
