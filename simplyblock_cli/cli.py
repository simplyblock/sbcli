...
class CLIWrapper(CLIWrapperBase):
    ...
    def init_storage_node__remove(self, subparser):
        subcommand = self.add_sub_command(subparser, 'remove', 'Removes a storage node from the cluster (inverse of cluster expansion). The node must be ONLINE and all other nodes ONLINE. Runs as a background task: shuts the node down, migrates any lvols and failed devices off it (status pending_migration), rewires LVS replicas onto other nodes, then marks the node removed.')
        subcommand.add_argument('node_id', help='Storage node id', type=str).completer = self._completer_get_sn_list
