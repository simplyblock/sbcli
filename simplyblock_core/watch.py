...
class ScopeWatch:
    ...
    def __init__(
            self, model_cls: type[Any], scope: Sequence[Any], entity_id: str | None,
            *, tail: int | None = None,
    ) -> None:
        self._model_cls = model_cls
        self._scope = tuple(scope)
        self._entity_id = entity_id
        # Bounds the list branch to the N most-recently-changed leaves (a live
        # tail) instead of the whole scope, for scopes with no natural bound
        # (e.g. an append-only per-cluster event log). Ignored for detail
        # watches, which already target exactly one leaf.
        self._tail = tail
    ...
    def _read(self, kv_store: Any) -> tuple[list[Any], bool]:
        ...
        if self._detail:
            ...
            # A leaf that ages out of a bounded tail simply stops appearing in
            # `cur` here, so it flows through the same `removed` path below as
            # a physical delete -- no separate eviction logic needed.
            range_kwargs = {'limit': self._tail, 'reverse': True} if self._tail else {}
            for k, v in kv_store.get_range_startswith(self._version_prefix, **range_kwargs):
...
def get_scope_watch(
        model_cls: type[Any], scope: Sequence[Any] = (), entity_id: str | None = None,
        *, tail: int | None = None,
) -> ScopeWatch:
    ...
    ``tail`` only takes effect the first time a given watch key is constructed
    -- callers requesting the same key must agree on it (in practice, one
    controller function always passes the same fixed value for a given model).
    """
    scope = tuple(scope)
    if entity_id is not None:
    ...
    with _scope_watches_lock:
        ...
        if sw is None:
            sw = _scope_watches[key] = ScopeWatch(model_cls, scope, entity_id, tail=tail)
        return sw
...
async def watch(
    ...
    the stream (a reconnect then hits the endpoint's regular 404). ``tail``
    bounds an unbounded list scope (e.g. an append-only log) to its N
    most-recently-changed entities instead of the whole scope; ignored with
    ``entity_id`` set. The first yielded batch is the initial state (all
    ``created``).
    ...
    main_sub = get_scope_watch(model_cls, scope, entity_id, tail=tail).subscribe(loop, notify)
...
def _compute_batch(
        models: list[Any], select: Select | None, prev: dict[str, dict],
) -> tuple[list[ChangeEvent], dict[str, dict]]:
