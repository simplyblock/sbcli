...
def recreate_lvstore_on_sec(secondary_node):
    ...
    for primary_node in primary_nodes:
        ...
        ret, err = secondary_node.rpc_client().jc_suspend_compression(jm_vuid=primary_node.jm_vuid, suspend=True)
        if not ret:
...
def create_lvstore(snode, ndcs, npcs, distr_bs, distr_chunk_bs, page_size_in_blocks, max_size):
    ...
    if snode.secondary_node_id:
        ...
        ret, err = sec_node.rpc_client().jc_suspend_compression(jm_vuid=snode.jm_vuid, suspend=True)
        if not ret:
            logger.info("Failed to resume JC compression adding task...")
