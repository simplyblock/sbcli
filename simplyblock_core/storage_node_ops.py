...
def generate_automated_deployment_config(max_lvol, max_prov, sockets_to_use, nodes_per_socket, pci_allowed, pci_blocked,
    ...
    if calculate_hp_only:
        minimum_hp_memory = 0
        cores_by_numa = utils.get_numa_cores()
        node_cores = utils.generate_core_allocation(cores_by_numa, sockets_to_use, nodes_per_socket, cores_percentage)
        node_index = 0
        ...
        for nid in sockets_to_use:
            for idx, core_group in enumerate(node_cores.get(nid, [])):
                distribution = utils.calculate_core_allocations(core_group["isolated"], number_of_alcemls + 1)
                ...
                small_pool_count, large_pool_count = utils.calculate_pool_count(number_of_alcemls +1, 2 * number_of_distribs,
                                                                          len(core_group["isolated"]),
                                                                          number_of_poller_cores or len(
                                                                              core_group["isolated"]))
                minimum_hp_memory += utils.calculate_minimum_hp_memory(small_pool_count, large_pool_count, max_lvol,
                                                                max_prov, len(core_group["isolated"]))

                node_index += 1
        logger.info(f"The minimum required huge pages on this host is: {utils.convert_size(minimum_hp_memory, 'MB')} MB")
        return True
    else:
