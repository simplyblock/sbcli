...
def _def_create_device_stack(device_obj, snode, force=False):
    ...
    if not namespace_found:
        ...
        _ = rpc_client.nvmf_subsystem_add_ns(subsystem_nqn, pt_name)
    else:
...
def device_set_failed(device_id):
    ...
    _ = device_set_state(device_id, NVMeDevice.STATUS_FAILED)
...
def remove_jm_device(device_id):
    db_controller = DBController()
    jm_device = None
    snode = None
    for node in db_controller.get_storage_nodes():
    ...
    if snode.status == StorageNode.STATUS_ONLINE:
        ...
        if snode.jm_device.pt_bdev:
            _ = rpc_client.bdev_PT_NoExcl_delete(snode.jm_device.pt_bdev)

        if snode.enable_ha_jm:
            _ = rpc_client.bdev_jm_delete(snode.jm_device.jm_bdev, safe_removal=True)
        else:
            _ = rpc_client.bdev_jm_delete(snode.jm_device.jm_bdev, safe_removal=False)

        _ = rpc_client.bdev_alceml_delete(snode.jm_device.alceml_bdev)

        # if len(snode.jm_device.jm_nvme_bdev_list) == 2:
        _ = rpc_client.bdev_raid_delete(snode.jm_device.raid_bdev)
    ...
    if snode.jm_device:
        ...
        if snode.jm_device.raid_bdev:
            ...
            for dev in snode.nvme_devices:
                ...
                if dev_part in bdevs_names and dev_part not in jm_nvme_bdevs:
