...
def delete_restore_s3_bdev(node, name: str) -> None:
    ...
    except (RPCException, Exception) as e:  # noqa: B014 - RPC clients raise broadly
...
def restore_backup(backup_id: str, lvol_name: str, pool_id_or_name: str,
    ...
    Args:
        key_wrapping_passphrase: Required when the backup's key is wrapped. Never
            persisted -- it is used to unwrap the key and discarded.
    ...
    if not tasks_controller.add_backup_restore_task(
            ...
            s3_config=s3_config.to_storage_dict() if s3_config is not None else None):
