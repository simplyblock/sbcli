...
from simplyblock_core.kms import KMSException, create_kms_connection, kek_path
...
def add_pool(name, pool_max, lvol_max, max_rw_iops, max_rw_mbytes, max_r_mbytes, max_w_mbytes, cluster_id,
    ...
    try:
        with create_kms_connection(cluster) as kms:
            kms.create_key_encryption_key(kek_path(cluster.get_id(), pool.get_id()))
...
def delete_pool(uuid):
    ...
    with create_kms_connection(cluster) as kms:
        try:
            kms.delete_key_encryption_key(kek_path(cluster.get_id(), pool.get_id()))
