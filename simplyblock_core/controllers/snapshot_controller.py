...
def clone(snapshot_id, clone_name, new_size=0, pvc_name=None, pvc_namespace=None, delete_snap_on_lvol_delete=False,
    ...
    if snap.lvol.crypto_bdev:
        with create_kms_connection(cluster) as kms:
            try:
                key1, key2 = kms.get_data_encryption_keys(snap.lvol.pool_uuid, snap.lvol.crypto_bdev, lvol=snap.lvol)
                kms.import_data_encryption_keys(lvol.pool_uuid, lvol.crypto_bdev, (key1, key2), lvol=lvol)
            except KMSException:
