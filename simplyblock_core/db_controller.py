...
class DBController(metaclass=Singleton):
    ...
    def get_caching_node_by_id(self, id)  -> CachingNode:
        ...
        if not ret:
            raise KeyError(f'CachingNode {id} not found')
        return ret[0]

    def get_caching_node_by_system_id(self, system_id)  -> CachingNode:
        nodes = CachingNode().read_from_db(self.kv_store)
        for node in nodes:
            if node.system_uuid == system_id:
                return node
        raise KeyError(f'No CachingNode for system {system_id} found')
    ...
    def get_caching_node_by_hostname(self, hostname)  -> CachingNode:
        ...
        for node in nodes:
            if node.hostname == hostname:
                return node
        raise KeyError(f'No CachingNode for host {hostname} found')
