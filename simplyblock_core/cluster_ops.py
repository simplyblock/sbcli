...
def cluster_activate(cl_id, force=False, force_lvstore_create=False) -> None:
    ...
    for snode in snodes:
        ...
        if snode.lvstore and force_lvstore_create is False:
            ...
            try:
                ret = storage_node_ops.recreate_lvstore(snode,cluster.first_time_activate)
            ...
            ret = storage_node_ops.create_lvstore(snode, cluster.distr_ndcs, cluster.distr_npcs, cluster.distr_bs,
                                              cluster.distr_chunk_bs, cluster.page_size_in_blocks, max_size,cluster.first_time_activate)
        snode = db_controller.get_storage_node_by_id(snode.get_id())
        if ret:
    ...
    for snode in snodes:
        ...
        ret = storage_node_ops.recreate_lvstore_on_sec(snode,cluster.first_time_activate)
    ...
    if not cluster.cluster_max_size:
        ...
        cluster.write_to_db(db_controller.kv_store)
    cluster.first_time_activate=False
