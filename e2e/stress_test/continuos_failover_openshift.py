...
class RandomMultiClientOpenshiftFailoverTest(TestLvolHACluster):
    """
    Extends the RandomMultiClientOpenshiftFailoverTest class to add a random failover and stress testing scenario.
    """
    ...
    def __init__(self, **kwargs):
        ...
        self.k8s_mgmt = True
        self.k8s_namespace = kwargs.get("k8s_namespace") or os.environ.get("SBCLI_NAMESPACE", "simplyblock")
        self.ssh_obj = SshUtils(bastion_server=self.bastion_server,
                                k8s_mgmt=self.k8s_mgmt,
                                k8s_namespace=self.k8s_namespace)
        self.test_name = "continuous_random_failover_openshift_ha"
    ...
    def create_lvols_with_fio(self, count):
        ...
        for i in range(count):
            ...
            if self.k8s_mgmt:
                self.ssh_obj.run_k8s(command=f"{self.base_cmd} lvol list")
            else:
